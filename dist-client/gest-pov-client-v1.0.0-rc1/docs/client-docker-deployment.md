...
## 4bis. Installation via Docker Hub public
...
```bash
cp .env.example .env
# Renseigner les secrets réels client (ne jamais committer .env)
...
> Les repositories Docker Hub sont publics (images téléchargeables), mais l'usage métier reste protégé par la licence (`gest_pov.lic`) et l'authentification applicative.
> Ne jamais versionner `.env`, `*.lic`, `backups/`, clés privées ou token Docker.
