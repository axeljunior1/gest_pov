...
## 17. Publication Docker Hub (release `1.0.0-rc1`)

Objectif : livraison client sans images `.tar`, via pull d'images publiques.
...
Notes sécurité :
- images publiques = téléchargement libre, mais API métier protégée par licence + auth.
- ne jamais inclure dans Git : `.env`, `*.lic`, `backups/`, clés privées, tokens Docker, archives `.tar`.
